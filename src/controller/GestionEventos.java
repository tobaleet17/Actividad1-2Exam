...
public class GestionEventos {
	...
	private void recuperar_todos() {
		...
		try {
			...
			for (int i=0;i<Array1.size();i++) {
				...
				sb.append("Titulo :");sb.append(Array1.get(i).getTitulo()+"\n"); //Añadimos el salto de línea para que no salgan los títulos juntos
				sb.append("Año :");sb.append(Array1.get(i).getAnoPubli()+"\n");
			}
	...
	public void recuperar_any()
	{
		...
		try {
			...
			ArrayList <Libro> Array1= model.Todos();
			StringBuilder sb = new StringBuilder();
			StringBuilder anyoGuardado = new StringBuilder();
			...
			for (int i=0;i<Array1.size();i++) {
				...
				if (anyoGuardado.append(view.getTextPublicacion().getText()).toString().equals(sb.append(Array1.get(i).getAnoPubli()).toString())){
					
					System.out.println("si que entra");
					view.getTextArea().setText("Los libros que son iguales : ");
					sb.append("El titulo del libro es: ");sb.append(sb.append(Array1.get(i).getTitulo()));
					
					
				}else {
					
					view.showError("No existe ningun libro con ese año de publi");
				}
	...
	private int call_buscarPalabraLong() {

		// TODO: Llamar a la función buscarPalabra de GestionDatos
		// TODO: Gestionar excepciones
		try {
			
			int num = Integer.parseInt(view.getPalabra().getText());
			int res=model.buscarPalabraLong(view.getFichero1().getText(), view.getPalabra().getText(), num);
			
			if (res==1) {
				view.getTextArea().setText("Se han encontrado palabras con esa long "+res);
				
			}else {
				view.showError("No se ha encontrado ninguna palabra con esa longitud");
			}
